...
genice2 IX --rep 5 4 4 -f exyz -s "${ICESEED}" > ice.temp 2> log.genice
../../tools/exyz2gpumd.py ice.temp -o ice.xyz
```

Then the sample need to be relaxed
...
## Production Run
Now that the ice sample is relaxed according to the NEP PES, it it ready to be excited

```bash
../../tools/wignerEXC2B.py relaxed.xyz -d excited.xyz -e 0.1 -vv > log.excite 2>&1
```

Running 'wignerEXC2B.py' with the '-h' flag will print usages and capabilities.  

Now the production can be carried out
